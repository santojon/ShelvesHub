#!/usr/bin/env bash
#
# Build, deploy and run Shelves Loader directly on a Steam Deck over SSH.
#
...
REMOTE_HOME_DIR="${DECK_DEPLOY_PATH:-/home/$DECK_USER/.local/share/shelves-loader}"
...
BIN="target/$TARGET/release/loader"
...
rsync -az -e "$RSH" "$BIN" "$DECK_USER@$DECK_HOST:$REMOTE_HOME_DIR/loader"
...
# assumes ~/.local/share/shelves-loader, which may differ from DECK_DEPLOY_PATH).
printf '%s\n' \
  "[Unit]" \
  "Description=Shelves Loader Service" \
  "After=network-online.target" \
  "" \
  "[Service]" \
  "ExecStart=$REMOTE_HOME_DIR/loader" \
  "Restart=always" \
  "RestartSec=10" \
  ...
  | "${SSH[@]}" "cat > /home/$DECK_USER/.config/systemd/user/shelves-loader.service"

# ── Enable Steam CEF remote debugging + (re)start the service ───────────────
echo "[i] Enabling CEF remote debugging and (re)starting service..."
"${SSH[@]}" "chmod +x '$REMOTE_HOME_DIR/loader' 2>/dev/null || true"
"${SSH[@]}" bash -s <<'REMOTE'
set -e
...
systemctl --user restart shelves-loader.service
echo "[remote] service status:"
systemctl --user --no-pager status shelves-loader.service | head -n 8 || true
REMOTE
...
echo "  • Tail logs:   ssh $DECK_USER@$DECK_HOST 'journalctl --user -u shelves-loader -f'"
